...
The provider returns one action from `start`, `finalize`, `validate`, `recover`, `maintainer_action`, `select_lineage`, `repair_authority`, `reconcile_finalize`, or `stop`. A consumer requests optional additive `eligibility.allowed_actions` and `eligibility.forbidden_actions` with `--action-eligibility`; each entry has a deterministic `reason_code`, and an eligible recovery carries its exact native lineage/revision/target binding. Existing v1 consumers that do not request the flag retain their historical strict payload. Consumers that request it MUST route only from this envelope, never infer an authorization, command binding, or template from prose, state, or a statusline. Missing worktrees, refs, targets, lineage, revisions, ambiguous/corrupt authority, and unverifiable materiality yield no authorization binding. Applicable non-terminal legacy-v1 authority always returns `stop`; it never recommends finalize or another mutation. A consumer MUST stop on ambiguity, corruption, invalidation, escalation, or `stop` and present the provider's required action. It MUST NOT silently choose, reset, quarantine, or create a lineage.
